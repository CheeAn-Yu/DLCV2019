...
def voc_eval(detlist,
    ...
    image_ids = [x[0] for x in detlist]
    confidence = np.array([float(x[1]) for x in detlist])

    #print('check image_ids:', image_ids)
    #print('check confidence: ', confidence)

    BB = np.array([[float(z) for z in x[2:]] for x in detlist])
...
def readdet(detpath, imagenames, classnames):
    ...
    # read dets
    for imagename in imagenames:
        detfile = detpath.format(imagename)
        with open(detfile, 'r') as f:
            lines = f.readlines()
        splitlines = [x.strip().split(' ') for x in lines]

        for line in splitlines:
            # line: x1, y1, x2, y1, x2, y2, x1, y2 class, conf
            classname = line[8]
            output_det[classname].append([imagename, line[9], line[0], line[1], line[4], line[5]])
