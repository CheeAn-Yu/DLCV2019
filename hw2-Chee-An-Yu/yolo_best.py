...
def decoder(pred):
    ...
    probs = []
    pred = pred.data
    pred = pred.view(14,14,26) #7x7x26
        ...
        for j in range(grid_num):
            for k in range(2):
                if mask[i,j,k] == 1:
                    ...
                    xmin = x_cen / 14 - pred[i,j,5*k+2]/ 2
                    ymin = y_cen / 14 - pred[i,j,5*k+3]/ 2
                    xmax = x_cen / 14 + pred[i,j,5*k+2]/ 2
                    ymax = y_cen / 14 + pred[i,j,5*k+3]/ 2
                    prob= pred[i,j,5*k+4]
                    ...
                    if float((prob*max_prob).item()) > 0.08:
                        boxes.append(box)
...
# if __name__ == '__main__':
    ...
    '''
import csv
import improve_model as models
import os
import sys
# import models
from dataloder_v5 import Load_test_data
...
# checkpoint_path="/home/robot/hw2-Chee-An-Yu/improve/yolo-4000.pth"
checkpoint_path = "./yolo-4000.pth"
...
# print(testset_loader)
with torch.no_grad():
    for step, batch_x in enumerate(testset_loader):
        ...
        f = open(join(sys.argv[2],test_fname[step]),"w")
