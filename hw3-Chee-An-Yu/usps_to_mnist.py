...
sourceTrainDataset = USPS(csv_file="./hw3_data/digits/usps/train.csv", root_dir="./hw3_data/digits/usps/train",transform=transforms.Compose([
...
sourceTestDataset =  USPS(csv_file="./hw3_data/digits/usps/test.csv", root_dir="./hw3_data/digits/usps/test",transform=transforms.Compose([
#                                transforms.Resize(image_size),
#                                transforms.CenterCrop(image_size),
                               transforms.ToTensor(),
                               transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5)),
...
targetTestDataset = MNIST(csv_file="./hw3_data/digits/mnistm/test.csv", root_dir="./hw3_data/digits/mnistm/test",transform=transforms.Compose([
#                                transforms.Resize(image_size),
#                                transforms.CenterCrop(image_size),
                               transforms.ToTensor(),
                               transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5)),
...
if ~tcuda.is_available():
    sourceCNN = torch.load('preTrainedCNNUSPS',map_location=lambda storage, loc: storage)
else:
    sourceCNN = torch.load('preTrainedCNNUSPS')

sourceCNN.eval()

targetCNN = LeNet()
if ~tcuda.is_available():
    targetCNN = torch.load('preTrainedCNNUSPS',map_location=lambda storage, loc: storage)
else:
    targetCNN = torch.load('preTrainedCNNUSPS')

targetCNN.train()

classifier = LeNetClassifier()
if ~tcuda.is_available():
    classifier = torch.load('preTrainedClassiferUSPS',map_location=lambda storage, loc: storage)
else:
...
torch.save(targetCNN, 'targetTrainedModelUSPStoMNIST')
