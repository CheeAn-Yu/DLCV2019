...
#     df = pd.DataFrame(csv,columns=['image_name','label'])
#     df.to_csv("./S2U_predict.csv",index=0)
...
if dataset_name == 'mnistm':
    ...
    model = torch.load("./targetTrainedModelMNISTtoSVHN")
    ...
    model = torch.load("./targetTrainedModelSVHNtoUSPS")
    classifier = torch.load("./preTrainedClassiferSVHN")

    test_dataset = USPS(root_dir=sys.argv[1],transform=transforms.Compose([
